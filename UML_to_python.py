...
class Item:
    def __init__(self,shippingWeight,description):
        self.shippingWeight = shippingWeight #measure in Kg
        self.description = description #Include price of an item,tax(%), number of item in stock
    def getPriceForQuantity(self):
        a = self.description
        a = a.split(",")
        return  float(a[0]) # a[0] is price of an item
    def getTax(self):
        a = self.description
        a = a.split(",")
        return float(a[1]) # a[1] is tax
    def inStock(self):
        a = self.description
        a = a.split(",")
        return int(a[2]) # a[2] is number of item in stock
